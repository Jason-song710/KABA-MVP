...
기본 설정은 관리자 키워드 사전에 등록된 키워드를 나라장터 `bidNtceNm` 공고명 검색 조건으로 넣어 수집한다. 기본 operation은 용역, 물품, 공사, 기타, 외자 공고이며 키워드별로 저장 또는 갱신된 공고마다 즉시 1차 키워드 점수를 산정한다. 중복 공고라도 마감일, 원문 링크, 첨부, 예산, 원본 응답이 바뀌면 `updated_count`로 집계한다.
