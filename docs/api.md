...
기본 설정은 관리자 키워드 사전에 등록된 키워드를 나라장터 `bidNtceNm` 공고명 검색 조건으로 넣어 수집한다. 최근 등록 기준과 마감/개찰 기준을 함께 조회하며, 마감/개찰 기준 조회 기간은 나라장터 입력 범위 제한을 피하기 위해 30일을 권장한다. 기본 operation은 용역, 물품, 공사, 기타, 외자 공고이며 키워드별로 저장 또는 갱신된 공고마다 즉시 1차 키워드 점수를 산정한다. 중복 공고라도 마감일, 원문 링크, 첨부, 예산, 원본 응답이 바뀌면 `updated_count`로 집계한다.

전체 공고 수집까지 추가로 실행하려면 서버 설정에서 `G2B_FULL_COLLECT_ENABLED=true`를 사용한다. 저장 후 `/notices`의 `q`, `category`, `active_only` 조건과 키워드 분류 결과로 검색, 핵심/관련/참고/제외, 내 회사 관련 공고를 구분한다.
