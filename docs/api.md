...
기본 설정은 최근 등록(`inqryDiv=1`)과 마감/개찰 기준(`inqryDiv=2`)을 함께 조회하며, `G2B_MAX_PAGES_PER_OPERATION`까지 페이지네이션한다. 중복 공고라도 마감일, 원문 링크, 첨부, 예산, 원본 응답이 바뀌면 `updated_count`로 집계한다.
